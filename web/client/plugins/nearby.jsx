...
const CONTROL_NAME = "NearBy_dialog";
...
const iconUrlMap = {
  ...
  // ...add all your categories and their icon URLs here
};


const NearBy = ({ text, dispatch, tempFeatures, features }) => {
    ...
    const handleDrawClick = (method) => {
        ...
        dispatch(changeDrawingStatus('start', method, 'NearBy', [], { stopAfterDrawing: true }));
    };
    ...
    return (
        <Dialog floating title="Nearby" className="nearby-dialog" style={{marginLeft: 650}}>
            <div style={{ minWidth: 300, background: '#f8fafd', borderRadius: 8, boxShadow: '0 2px 8px #0001', padding: 12, border: '1px solid #bcd', display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                {DrawSupport && <DrawSupport drawOwner="NearBy" />}
                <button
                ...
                <button
                    ...
                    onClick={() => {
                        dispatch(changeDrawingStatus('clean', '', 'NearBy', [], {}));
                        const selectedCat = POI_CATEGORIES.find(cat => cat.key === selectedCategory);
...
/**
 * NearByPlugin. A dialog window that can be opened from the burger menu.
 * This is a good point to start developing your plugin.
 * - Connect the state to NearBy component
 * - Connect actions to dispatch to the NearBy component (create an actionCreators file for custom actions)
 * - Edit your reducers/NearBy.js file to handle the `NearBy` piece of global redux state.
 * - Add epics...
 */
export default createPlugin("NearBy", {
    ...
    reducers: {
        NearBy: reducer // REDUCER will be used to create the `NearBy` part of global redux state (keys of the "reducers" are pieces of state)
    }
});